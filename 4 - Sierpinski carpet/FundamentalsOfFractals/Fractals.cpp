...
ID3D11Buffer*           g_pCBShaderParameters = NULL;
XMMATRIX                g_World1;
XMMATRIX                g_World2;
XMMATRIX                g_View;
...
HRESULT InitWindow( HINSTANCE hInstance, int nCmdShow )
{
    ...
    g_hWnd = CreateWindow( L"TutorialWindowClass", L"Fundamentals of Fractals",
                           WS_OVERLAPPEDWINDOW,
...
HRESULT InitDevice()
{
    ...
    hr = CompileShaderFromFile( L"Fractals.fx", "VS", "vs_4_0", &pVSBlob );
    if( FAILED( hr ) )
    {
        MessageBox( NULL,
    ...
    hr = CompileShaderFromFile( L"Fractals.fx", "PS_Sierpinski", "ps_4_0", &pPSBlob );
    if( FAILED( hr ) )
    {
        MessageBox( NULL,
                    L"The FX file cannot be compiled.  Please run this executable from the directory that contains the FX file.", L"Error", MB_OK );
	...
	bd.Usage = D3D11_USAGE_DEFAULT;
    bd.ByteWidth = sizeof(CBShaderParameters);
    bd.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
    bd.CPUAccessFlags = 0;
    hr = g_pd3dDevice->CreateBuffer( &bd, NULL, &g_pCBShaderParameters );
    if( FAILED( hr ) )
        return hr;
	...
	XMVECTOR Eye = XMVectorSet( 0.0f, 1.0f, -120.0f, 0.0f );
	XMVECTOR At = XMVectorSet( 0.0f, 1.0f, 0.0f, 0.0f );
	XMVECTOR Up = XMVectorSet( 0.0f, 1.0f, 0.0f, 0.0f );
	g_View = XMMatrixLookAtLH( Eye, At, Up );

    // Initialize the projection matrix
	g_Projection = XMMatrixPerspectiveFovLH( XM_PIDIV2, width / (FLOAT)height, 0.01f, 10000.0f );
...
void CleanupDevice()
{
	...
	if( g_pCBShaderParameters ) g_pCBShaderParameters->Release();
...
// Based on the code from Wikipedia: http://en.wikipedia.org/wiki/Sierpinski_carpet
